...
from geometry_msgs.msg import Pose as GPose, Vector3, Point, Quaternion
...
class Pose(GPose):
    ...
    def parse(node):
        ...
        if node.hasAttribute("xyz"):
            ...
            pose.position = Point(*map(float, xyz))
        if node.hasAttribute("rpy"):
            rpy = node.getAttribute('rpy').split()
            pose.orientation = Quaternion(*tf.transformations.quaternion_from_euler(*map(float, rpy)))
        return pose
    ...
    def to_xml(self, doc):
        ...
        if self.orientation:
            quat = self.orientation.x, self.orientation.y, self.orientation.z, self.orientation.z
            set_attribute(xml, 'rpy', tf.transformations.euler_from_quaternion(quat))
        return xml
